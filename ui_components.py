...
def render_sidebar():
    ...
    with st.sidebar:
        st.header("🎮 시스템 제어판")
        st.radio("알고리즘 선택", ["정수계획법(IP)", "선형계획법(LP)"], key="opt_mode")

        st.markdown("---")
        st.subheader("🏭 공급망 전략")
        st.toggle("외주 하청 허용", key="enable_sub")

        st.markdown("---")
        st.subheader("⏱️ 공정 효율 및 기초 제약")
        st.number_input("제품당 표준 작업 시간 (Hr)", min_value=1.0, step=0.1, key="std_time")
        st.slider("월간 가동 일수 (일)", 1, 30, key="working_days")
        st.slider("인당 월간 초과근무 한도 (Hr)", 0, 100, key="ot_limit")

        st.markdown("---")
        st.subheader("🛡️ 운영 정책 및 생산 제약")
        st.number_input("최대 허용 가동률 (%)", min_value=1.0, max_value=100.0, step=0.5, key="max_util")
        st.number_input("최소 유지 재고량 (ea)", min_value=0, key="min_inv")

        st.markdown("---")
        st.subheader("💰 운영 비용 설정 (단위: 천원)")
        # 강의록 수리 모델 기준 단위 복구
        costs = [
            ("v_c_reg", "정규 임금 (천원/인/월)"), 
            ("v_c_ot", "초과 수당 (천원/시간)"), 
            ("v_c_h", "고용 비용 (천원/인)"), 
            ("v_c_l", "해고 비용 (천원/인)"),
            ("v_c_inv", "재고 유지비 (천원/개/월)"), 
            ("v_c_back", "부재고 비용 (천원/개/월)"), 
            ("v_c_mat", "제품 재료비 (천원/개)"), 
            ("v_c_sub", "외주 하청비 (천원/개)")
        ]
        for k, lbl in costs:
        ...
        st.text_input("수요 예측 (쉼표 구분, ea)", key="demand_raw")
    ...
    return demand, st.session_state.enable_sub, st.session_state.std_time, st.session_state.working_days, st.session_state.ot_limit
...
def render_supply_demand_tab(m, utils, demand):
    ...
    if min_i > 0: fig.add_hline(y=min_i, line_dash="dot", line_color="#27AE60", annotation_text="최소 재고 가이드", yref="y2")
    fig.update_layout(yaxis2=dict(overlaying='y', side='right'), barmode='stack', hovermode="x unified")
    st.plotly_chart(fig, use_container_width=True)
    st.subheader("👷 인력 최적 배치 트렌드")
    worker_counts = [int(m.W[t]()) for t in range(1, len(demand) + 1)]
    fig_w = go.Figure()
    fig_w.add_trace(go.Bar(x=[f"{t}월" for t in range(1,len(demand)+1)], y=[int(m.H[t]()) for t in range(1,len(demand)+1)], name="신규 고용 (+)", marker_color="#3498DB"))
    fig_w.add_trace(go.Bar(x=[f"{t}월" for t in range(1,len(demand)+1)], y=[-int(m.L[t]()) for t in range(1,len(demand)+1)], name="해고 처리 (-)", marker_color="#E67E22"))
    fig_w.add_trace(go.Scatter(x=[f"{t}월" for t in range(1,len(demand)+1)], y=worker_counts, mode="lines+markers", name="총 가동 인원", line=dict(color="#1ABC9C", width=4)))
