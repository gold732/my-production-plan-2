...
def render_supply_demand_tab(m, utils, demand):
    ...
    if current_min_inv > 0:
        fig.add_hline(y=current_min_inv, line_dash="dot", line_color="#27AE60", annotation_text="최소 재고", yref="y2")
...
def render_risk_efficiency_tab(m, utils, demand):
    ...
    with c2:
        st.markdown("##### ⏳ 연장근로 활용도")
        ot = [m.O[t]() for t in range(1, len(demand)+1)]
        st.plotly_chart(px.bar(x=[f"{t}월" for t in range(1,len(demand)+1)], y=ot, marker_color='#E74C3C'), use_container_width=True)
