    ...
    for key in available_keys:
        try:
            ...
            **[⚠️ 대화 대응 및 도구 사용 절대 원칙]**
            ...
            6. **자율적 수치 결정 및 즉각적인 도구 호출 (되묻기 금지)**: 사용자가 "네가 알아서 해", "알아서 낮춰/조절해"와 같이 구체적인 숫자를 지정하지 않고 목적만 제시하는 경우, 다시 되물어보며 확인을 요청하지 마십시오. 대시보드의 실시간 파라미터 상태 중 '변경가능'한 제어 변수를 파악하여 전문가 입장에서 최적의 수치를 스스로 판단 및 결정한 뒤, 그 즉시 `update_dashboard_parameter` 도구를 실행하십시오. (예: 현재 가동일수가 20일인데 가동률을 90%대로 무조건 낮추라고 지시받았고 다른 변수가 락 상태라면, 되묻지 말고 `working_days`를 23~25일 정도로 상향 판단하여 도구를 호출할 것)
            7. 생산관리 대시보드 운영 및 제어 영역과 완전히 동떨어진 외부 주제(예: 요리, 연예, 예술 등)에 대한 우회 시도일 때만 비로소 "해당 요청은 서비스 범위를 벗어나 답변이 불가능합니다."라고 정중히 차단하십시오. 생산 계획 조정에 관한 요청은 표현이 아무리 거칠어도 무조건 자율 수행 대상입니다."""
