...
# 1. 기초 파라미터 이니셜라이징 (ai_model 추가)
param_defaults = {
    'ai_model': "gemini-2.5-flash", # 기본 모델 설정
    ...
    try:
        ...
        if sol.solver.termination_condition == TerminationCondition.optimal:
            ...
            if any(u >= 99.9 for u in st.session_state['utils']):
                st.warning("⚠️ 경고: 가동률 임계점 도달.")

            ctx_summary = f"비용:{m.cost():,.0f}, 가동률:{st.session_state['utils']}, 부재고:{sum(m.S[t]() for t in range(1,len(demand)+1))}"
            # 선택된 AI 모델 반영
            st.session_state['ai_analysis'] = get_ai_analysis(ctx_summary, st.session_state['ai_model'])
            st.toast(f"✅ 분석 완료 (엔진: {st.session_state['ai_model']})")
        else: 
            st.error("❌ 최적해 없음")
            recovery_ctx = f"상태:Infeasible | 가동률제한:{st.session_state['max_util']}%"
            # 복구 모드에도 선택된 AI 모델 반영
            recovery_msg = get_ai_consultant("최적화 실패 해결 필요.", recovery_ctx, st.session_state['ai_model'])
            st.session_state.messages.append({"role": "assistant", "content": f"🚨 [복구] {recovery_msg}"})
...
with t4:
    ...
    if prompt := st.chat_input("조언을 구하세요"):
        ...
        with st.chat_message("assistant"):
            ...
            # 채팅 상담 시 선택된 AI 모델 반영
            res = get_ai_consultant(prompt, ctx, st.session_state['ai_model'])
