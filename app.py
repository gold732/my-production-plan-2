...
# [🚨 수정]: 멀티라인 임포트 구문을 사용하여 SyntaxError 방지 및 함수명 100% 동기화
...
def run_optimization():
    ...
    try:
        ...
        if sol.solver.termination_condition == TerminationCondition.optimal:
            ...
            # 최적화 직후 AI 분석 보고서 생성
            ctx_ana = f"비용:{m.cost():,.0f}, 가동률:{st.session_state['utils']}, 부재고:{sum(m.S[t]() for t in range(1,len(demand)+1))}"
            st.session_state['ai_analysis'] = get_ai_analysis(ctx_ana)
            st.toast("✅ 전략적 생산계획 수립 완료!")
        else: st.error("❌ 최적해를 찾지 못했습니다. 제약 조건을 완화해 보십시오.")
...
with t2:
    ...
    else: st.info("먼저 [공급망 운영] 탭에서 수립 실행을 해주세요.")
...
with t4:
    ...
    if prompt := st.chat_input("질문을 입력하세요"):
        ...
        with st.chat_message("assistant"):
            ...
            cost_val = st.session_state['res'].cost() if st.session_state['success'] else "N/A"
            ctx = f"현재 가동률:[{u_str}] | 총비용:{cost_val} | 락상태:전략파라미터 고정됨"
