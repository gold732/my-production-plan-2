...
for pk in initial_locked_keys:
    lock_key = f"lock_{pk}"
    if lock_key not in st.session_state: st.session_state[lock_key] = True
    ...
    try:
        ...
        if sol.solver.termination_condition == TerminationCondition.optimal:
            ...
            st.session_state['utils'] = [
                (m.P[t]()*st.session_state['std_time']/(8*st.session_state['working_days']*m.W[t]())*100 if m.W[t]() > 0 else 0) 
                for t in range(1, len(demand)+1)
            ]
            st.toast("✅ 생산계획 수립 완료!")
        else: st.error("❌ 최적해 없음. 제약을 완화하십시오.")
...
# 4. 4단 탭 UI 배치
tab_list = st.tabs(["📊 공급망 운영", "📉 리스크/효율", "📋 데이터 마스터", "💬 AI 전략 상담방"])
...
with tab_list[3]:
    ...
    if prompt := st.chat_input("조언을 구하세요"):
        ...
        with st.chat_message("assistant"):
            ctx = f"가동률:{st.session_state.get('utils')} | 락상태:{ {k: st.session_state.get(f'lock_{k}') for k in param_defaults} }"
