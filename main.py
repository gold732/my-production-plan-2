...
if 'res_model' not in st.session_state: st.session_state.res_model = None
...
with st.sidebar:
    ...
    trend = st.selectbox("시장 트렌드", ["평시", "호황", "불황", "봄철 수요 폭증"])
    ...
    if st.button("🪄 AI에게 수요 보정 요청"):
        ...
        st.info(f"AI 추천: {ai_suggestion}")

    st.markdown("---")
    st.subheader("⏱️ 공정 효율 및 비용 설정")
    ...
    if st.button("🚀 최적 생산계획 수립 실행"):
        with st.spinner('최적화 계산 중...'):
            ...
            if m and sol.solver.termination_condition == TerminationCondition.optimal:
                st.session_state.res_model = m
                st.session_state.utils = [(m.P[t]() * std_time / (8 * working_days * m.W[t]()) * 100) if m.W[t]() > 0 else 0 for t in range(1, 7)]
                st.toast("✅ 최적화 성공!")
            else:
                st.error("❌ 최적해를 찾지 못했습니다. 제약 조건을 조정하세요.")

    if st.session_state.res_model:
        m = st.session_state.res_model
        # KPI 메트릭
...
with tab2:
    if st.session_state.res_model:
        st.subheader("🚩 AI 리스크 진단 보고")
        risk_text = get_risk_analysis(st.session_state.utils, enable_sub)
        st.warning(risk_text)
        
        fig_u = px.area(x=list(range(1,7)), y=st.session_state.utils, title="생산 가동률 추이 (%)", markers=True)
        fig_u.add_hline(y=100, line_dash="dot", line_color="red")
        st.plotly_chart(fig_u, use_container_width=True)

with tab3:
    st.subheader("💬 AI 전략 상담방")
    if st.button("📄 AI 경영 요약 보고서 생성"):
        if st.session_state.res_model:
            m = st.session_state.res_model
            report = get_ai_consultant("현재 계획의 핵심 성과와 개선점을 경영진 보고용으로 요약해줘.", f"총비용:{m.cost():,.0f}, 가동률:{st.session_state.utils}")
            st.success(report)
    ...
    if prompt := st.chat_input("계획에 대해 질문하세요."):
        ...
        ctx = f"총비용:{st.session_state.res_model.cost():,.0f}, 외주사용:{enable_sub}" if st.session_state.res_model else "데이터 없음"
